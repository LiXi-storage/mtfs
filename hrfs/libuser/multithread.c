...
struct hrfs_hlist_head thread_head;
...
int add_thread_info(thread_info_t *thread_info)
{
	...
	hrfs_hlist_add_head(&(thread_info->hnode), &thread_head);
out:
	return ret;
}
...
{
	struct hrfs_hlist_node *pos = NULL;
	thread_info_t *tpos = NULL;
	int ret = -1;
	
	hrfs_hlist_for_each_entry(tpos, pos, &thread_head, hnode) {
		if (tpos == thread_info) {
			hrfs_hlist_del(pos);
			ret = 0;
			break;
...
thread_info_t *find_thread_info(const char *identifier)
{
	...
	hrfs_hlist_for_each_entry(thread_info, pos, &thread_head, hnode) {

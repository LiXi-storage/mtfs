...
/* Is this a hrfs fs? */
int hrfsapi_is_hrfs_mnt(struct mntent *mnt)
{
	return (strcmp(mnt->mnt_type, HRFS_FS_TYPE) == 0);
}

/* Is this a hrfs-hidden fs? */
int hrfsapi_is_hrfs_hidden_mnt(struct mntent *mnt)
{
	return (strcmp(mnt->mnt_type, HRFS_HIDDEN_FS_TYPE) == 0);
}

int hrfs_api_hide(const char *path, int unhide, hrfs_param_t *param)
{
	FILE *fp = NULL;
	int ret = 0;
	...
	if (out_len <= 0) {
		HERROR("%s isn't mounted on hrfs\n", path);
		ret = -EINVAL;
		goto free_device;
	}

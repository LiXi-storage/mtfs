...
{
	int ret = 0;
	ret = lowerfs_inode_get_flag_xattr(inode, hrfs_flag, XATTR_NAME_HRFS_FLAG_NTFS3G);
	HRETURN(ret);
}
...
{
	int ret = 0;
	ret = lowerfs_inode_set_flag_xattr(inode, hrfs_flag, XATTR_NAME_HRFS_FLAG_NTFS3G);
	HRETURN(ret);
}

...
struct pin {
	 ...
	 */
	struct hrfs_list_head prevs;
	struct pin *next;

	/*
	 * For place in prevs field of the next pin 
	 */
	struct hrfs_list_head pin_list;
};
...
struct graph {
	struct hrfs_list_head subcomponents;
};
...
struct component {
	...
	struct hrfs_list_head component_list; /* For placing in parent graph */

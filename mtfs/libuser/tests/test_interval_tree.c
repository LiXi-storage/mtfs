...
static enum mtfs_interval_iter cb(struct mtfs_interval_node *n, void *args)
{
...
static int it_test_search(struct mtfs_interval_node *root)
{
        struct it_node *n;
        ...
        while (times--) {
                ...
                mtfs_interval_search(root, &ext, cb, NULL);
...
static int it_test_iterate(struct mtfs_interval_node *root)
{
        int i;
        ...
        it_test_clear();
        mtfs_interval_iterate(root, cb, NULL);
...
static int it_test_iterate_reverse(struct mtfs_interval_node *root)
{
        int i;

        dprintf("\n\niterate reverse testing start..\n");
        it_test_clear();
        mtfs_interval_iterate_reverse(root, cb, NULL);
...
static int it_test_find(struct mtfs_interval_node *root)
{
        int idx;
        struct mtfs_interval_node_extent *ext;

        dprintf("\nmtfs_interval_find testing start ..\n");
        for (idx = 0; idx < it_count; idx++) {
                ...
                if (!mtfs_interval_find(root, ext))
                        error("mtfs_interval_find, try to find "__S"\n", __F(ext));
        }
        return 0;
}

/* sanity test is tightly coupled with implementation, so when you changed
 * the mtfs_interval tree implementation, change this code also. */
static enum mtfs_interval_iter sanity_cb(struct mtfs_interval_node *node, void *args)
{
        __u64 max_high = node->in_max_high;
        struct mtfs_interval_node *tmp, *parent;
        int left = 1, has = 0, nr = 0;

        parent = node->in_parent;
        node->in_parent = NULL;
        mtfs_interval_for_each(tmp, node) {
                if ((left && node_compare(tmp, node) > 0) ||
                    (!left && node_compare(tmp, node) < 0))
                        error("mtfs_interval tree sanity test\n");
...
static int it_test_sanity(struct mtfs_interval_node *root)
{
        it_test_clear();
        mtfs_interval_iterate(root, sanity_cb, NULL);
        return 0;
}

static int it_test_search_hole(struct mtfs_interval_node *root)
{
        int i, count = 10;
        struct mtfs_interval_node_extent ext, ext2;
        struct it_node *n;
        __u64 low = 0, high = ~0;
        ...
        do {
                ...
                ext.end = ext.start;
        } while (mtfs_interval_is_overlapped(root, &ext));
        ext2 = ext;

        mtfs_interval_expand(root, &ext, NULL);
...
static enum mtfs_interval_iter perf_cb(struct mtfs_interval_node *n, void *args)
{
...
static int it_test_performance(struct mtfs_interval_node *root, unsigned long len)
{
        int i = 0, mtfs_interval_time, list_time;
        struct mtfs_interval_node_extent ext;
        struct it_node *n;
        ...
        mtfs_list_for_each_entry(n, &header, list) {
                if (extent_overlapped(&ext, &n->node.in_extent)) {
                        count = LOOP_COUNT;
                        while (count--);
                        contended_count++;
        ...
        contended_count = 0;
        gettimeofday(&start, NULL);
        mtfs_interval_search(root, &ext, perf_cb, &contended_count);
        gettimeofday(&end, NULL);
        mtfs_interval_time = tv_delta(&start, &end);
        ...
        printf("\tList vs Int. search: \n\t\t"
               "(%d vs %d)ms, %d contended lock.\n",
                list_time, mtfs_interval_time, contended_count);

        return 0;
}

static struct mtfs_interval_node *it_test_helper(struct mtfs_interval_node *root)
{
        int idx, count = 0;
        struct it_node *n;
        ...
        while (count--) {
                ...
                n = &it_array[idx];
                if (n->valid) {
                        if (!mtfs_interval_find(root, &n->node.in_extent))
                                error("Cannot find an existent node\n");
                        dprintf("Erasing a node "__S"\n",
                                __F(&n->node.in_extent));
                        mtfs_interval_erase(&n->node, &root);
                        n->valid = 0;
                        mtfs_list_del_init(&n->list);
                } else {
                        __u64 low, high;
                        ...
                        if (high > max_count)
                                high = max_count;
                        mtfs_interval_set(&n->node, low, high);
                        while (mtfs_interval_insert(&n->node, &root))
                                mtfs_interval_set(&n->node, low, ++high);
                        dprintf("Adding a node "__S"\n",
                                __F(&n->node.in_extent));
                        n->valid = 1;
                        mtfs_list_add(&n->list, &header);
                }
        }
...
static struct mtfs_interval_node *it_test_init(int count)
{
        int i;
        ...
        it_count = count;
        it_array = (struct it_node *)calloc(count, sizeof(struct it_node));
        if (it_array == NULL)
        ...
        for (i = 0; i < count; i++) {
                ...
                n->valid = 1;
                if (i == 0)
                        mtfs_list_add_tail(&n->list, &header);
                else
                        mtfs_list_add_tail(&n->list, &it_array[rand()%i].list);
        }

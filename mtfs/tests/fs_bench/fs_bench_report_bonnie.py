...
query = """select max(bench_id) from fs_bench where bench_program = 'bonnie'""";
cursor=db.cursor();
...
for i in data:
	...
	for column in columns:
		...
		if column not in ["fs_type", "file_size", "num_files", "fs_version", "kernel_version", "display_case_ratio"]:
			try:

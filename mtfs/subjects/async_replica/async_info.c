...
struct msubject_async_info *masync_info_init(struct super_block *sb)
{
	...
	MTFS_INIT_LIST_HEAD(&info->msai_lru_chunks);
	mtfs_spin_lock_init(&info->msai_lru_chunk_lock);
	atomic_set(&info->msai_lru_chunk_number, 0);
...
force_wait:
	...
	MASSERT(mtfs_list_empty(&info->msai_lru_chunks));
	MASSERT(atomic_read(&info->msai_lru_chunk_number) == 0);

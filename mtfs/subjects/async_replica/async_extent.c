 ...
 * Make sure reference is 1 or 2.
 */
int masync_extent_flush(struct masync_extent *async_extent,
			char *buf,
                        int buf_size)
{
	struct masync_bucket *bucket = async_extent->mae_bucket;
	...
	if (atomic_read(&async_extent->mae_reference) < 2) {
		/* Already remove from tree */
		MASSERT(atomic_read(&async_extent->mae_reference) == 1);
		up(&bucket->mab_lock);
		goto out;
	}
